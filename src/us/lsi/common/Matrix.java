...
import us.lsi.common.tuples.IntPair;
import us.lsi.common.views.View2;
import us.lsi.common.views.View4;
...
public class Matrix<E> {
    ...
    public Matrix<E> view(Integer nv) {
        ...
        switch (nv) {
            case 0:
                ...
                break;
            case 1:
                r = view(0, nc, nf, this.nc - nc);
                break;
            case 2:
                r = view(nf, 0, this.nf - nf, nc);
                break;
            case 3:
                r = view(nf, nc, this.nf - nf, this.nc - nc);
                break;
            default:
                Preconditions.checkArgument(false, "Opción no válida");
        }
        return r;
    }
    ...
    public static <E> Stream<Matrix<E>> allSubMatrix(Matrix<E> m) {
        return Stream2.allQuartets(0, m.nf(), 0, m.nc(), 1, m.nf(), 1, m.nc()).filter(q -> q.third() <= m.nf() - q.first() && q.fourth() <= m.nc() - q.second()).map(q -> m.view(q.first(), q.second(), q.third(), q.fourth()));
    }

    public static <E> Boolean allElements(Matrix<E> m, Predicate<E> pd) {
        return Stream2.allPairs(m.nf(), m.nc()).map(m::get).allMatch(pd);
    }

    public static <E> Boolean anyElements(Matrix<E> m, Predicate<E> pd) {
        return Stream2.allPairs(m.nf(), m.nc()).map(m::get).anyMatch(pd);
    }

    public void print(String title) {
        System.out.println(String.format("%n%s = (nf = %d, nc = %d, fv = %d, cv = %d)%n", title, this.nf, this.nc, this.fv, this.cv));
    ...
    public static <E extends FieldElement<E>> Matrix<E> multiply_r(Matrix<E> m1, Matrix<E> m2) {
        ...
        Preconditions.checkArgument(ch, String.format("No se cumplen condiciones de multiplicación = (in1.nc = %d, in2.nf = %d)", m1.nc, m2.nf));
