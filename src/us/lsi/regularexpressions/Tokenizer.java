...
public class Tokenizer {
    ...
    private static final String identifier = "[_a-zA-Z][a-zA-Z0-9]*";
    private static final String operatorOrSeparator =
            "\\*=?|\\+=?|\\-=?|/=?|!=?|(==?)|(<=?)|(>=?)|\\?\\:?|[\\^\\(\\)\\{\\}\\,\\;]";
    private static final String symbol = "[$&@]";
    private static final Pattern pSpace = Pattern.compile(space);
    ...
    public void error(TokenType... s) {
        ...
        Preconditions.checkState(sl.contains(r.type),
                String.format("%nSe esperaba %s %ny se ha encontrado %s en la posición %n%s",
                        sl,
                        currentToken(),
                        suffix()));
    }
