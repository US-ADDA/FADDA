...
public class Stream2 {
    ...
    public static <E> Boolean allEquals(Stream<E> st) {
        ...
        return st.peek(e -> {
                    if (first.value() == null) first.setValue(e);
                })
     ...
     * uno start uno los elementos de las secuencias de entrada
     */
    ...
    public static <T> Stream<Pair<T, T>> cartesianProduct(Collection<T> s1) {
        return cartesianProduct(s1, s1, Pair::of);
    }
    ...
    public static <E> Stream<Pair<E, E>> consecutivePairs(Stream<E> sm) {
        ...
        return sm.map(e -> Pair.of(rf.setValue(e), e)).filter(p -> p.first() != null);
    }
    ...
    public static Stream<IntTriplet> allTrios(Integer n1, Integer n2, Integer m1, Integer m2, Integer r1, Integer r2) {
        return IntStream.range(n1, n2).boxed()
                ...
                .flatMap(i -> IntStream.range(m1, m2).boxed()
                        ...
                        .flatMap(j -> IntStream.range(r1, r2).boxed().map(k -> IntTriplet.of(i, j, k))));
    }

    public static Stream<IntTriplet> allTrios(Integer n, Integer m, Integer r) {
    ...
    /**
     * @param s1  Una secuencia de streams
     ...
     * @param fr  Una funci�n que calcula un nuevo valor start partir  de uno procedente de stream1 y otro del stream2
     ...
     * aritm�tica desde start hasta end con raz�n sep sin incluir el valor end.
     */
    public static IntStream range(Integer a, Integer b, Integer c) {
        Preconditions.checkArgument(a.equals(b) || (b - a) * c > 0);
        int hasta = (b - a) / c;
        if ((b - a) % c == 0) {
            hasta = hasta - 1;
        }
     ...
     * @param c Un entero
     * @return Un stream de enteros que forman una progresi�n
     * aritm�tica desde start hasta end con raz�n sep incluyendo el valor end se es posible
     */
    public static IntStream rangeClosed(Integer a, Integer b, Integer c) {

        Preconditions.checkArgument(Objects.equals(a, b) || (b - a) * c > 0);
        int hasta = (b - a) / c;
        return IntStream.rangeClosed(0, hasta).map(x -> a + c * x);
    }
    ...
    public static <T> T elementRandom(Stream<T> st) {
        List<T> ls = st.toList();
        Integer n = ls.size();
        return ls.get(Math2.getRandomInteger(0, n));
    }
    ...
    public static Stream<String> split(String s, String delim) {
        ...
        return Arrays.stream(r).map(String::trim)
                .filter(x -> x.length() > 0);
    }
