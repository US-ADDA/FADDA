...
import com.fadda.graph.hypergraphs.GraphTree;
import com.fadda.graph.hypergraphs.SimpleHyperEdge;
import com.fadda.graph.hypergraphs.SimpleVirtualHyperGraph;
import com.fadda.graph.hypergraphs.VirtualHyperVertex;
...
public class DynamicProgramming<V extends VirtualHyperVertex<V, E, A>,
    ...
    public final SimpleVirtualHyperGraph<V, E, A> graph;
    public final Map<V, Sp<E>> solutionsTree;
    public final Boolean withGraph = false;
    private final PDType type;
    ...
    DynamicProgramming<V, E, A> dynamicProgrammingSearch(
            ...
            PDType type) {
        return new DynamicProgramming<>(graph, type);
    }
    ...
    public Sp<E> search(V actual) {
        ...
        if (this.solutionsTree.containsKey(actual)) {
        ...
        } else if (Boolean.TRUE.equals(graph.isBaseCase(actual))) {
            ...
            for (E edge : graph.edgesOf(actual)) {
                ...
                Sp<E> spa = null;
                if (spNeighbors != null) {
                    List<Double> solutions = spNeighbors.stream().map(Sp::weight).toList();
                    spa = Sp.of(graph.getEdgeWeight(edge, solutions), edge);
                }
                sps.add(spa);
                if (Boolean.TRUE.equals(this.withGraph)) this.completeGraph(actual, edge, neighbords);
            }
            r = sps.stream().filter(Objects::nonNull).min(this.comparatorSp).orElse(null);
            this.solutionsTree.put(actual, r);
    ...
    public record VertexGraph<V, E>(V vertex, E edge, DPTipoVertex tipo) {
        ...
        private static <V, E> Boolean optimalEdge(Graph<VertexGraph<V, E>, SimpleEdge<VertexGraph<V, E>>> graph,
            ...
            return graph.edgesOf(vf).stream()
                    ...
                    .map(VertexGraph::vertex)
                    .allMatch(optimalVertex::contains);
        }
        ...
        public static <V, E> Map<String, Attribute> colorEdge(Graph<VertexGraph<V, E>, SimpleEdge<VertexGraph<V, E>>> graph,
            ...
            if (Boolean.TRUE.equals(optimalEdge(graph, edge, optimalVertex))) {
        ...
        public enum DPTipoVertex {Vertex, Edge}
