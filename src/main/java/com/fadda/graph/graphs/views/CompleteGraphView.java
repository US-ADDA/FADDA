...
public class CompleteGraphView<V, E, G extends Graph<V, E>> implements Graph<V, E> {
    ...
    private final Supplier<E> edgeWeightFactory;
    ...
    public static <V, E, G extends Graph<V, E>> CompleteGraphView<V, E, G> of(
            G graph,
            Supplier<E> edgeWeightFactory) {
        return new CompleteGraphView<>(graph, edgeWeightFactory);
    }
    ...
    public Set<E> edgeSet() {
        return graph.vertexSet().stream().flatMap(x -> edgesOf(x).stream()).collect(Collectors.toSet());
    }
