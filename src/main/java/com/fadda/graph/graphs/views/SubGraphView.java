...
public class SubGraphView<V, E, G extends Graph<V, E>> implements Graph<V, E> {
    ...
    private final Predicate<V> vertices;
    private final Predicate<E> edges;

    private SubGraphView(G graph, Set<V> vertices) {
        super();
        this.graph = graph;
        this.vertices = vertices::contains;
        this.edges = e -> true;
    }
    ...
    public static <V, E, G extends Graph<V, E>> SubGraphView<V, E, G> of(G graph, Predicate<V> vertices, Predicate<E> edges) {
        ...
        return new SubGraphView<>(graph, vertices, edges);
    }


    public static <V, E, G extends Graph<V, E>> SubGraphView<V, E, G> of(G graph, Set<V> vertices) {

        return new SubGraphView<>(graph, vertices);
    }
    ...
    public Set<E> edgeSet() {
        return graph.edgeSet().stream().filter(this::containsEdge).collect(Collectors.toSet());
    }

    public Set<E> edgesOf(V v) {
        return graph.edgesOf(v).stream()
                .filter(this::containsEdge)
                .collect(Collectors.toSet());
    }

    public Set<E> getAllEdges(V v1, V v2) {
        return graph.getAllEdges(v1, v2).stream().filter(this::containsEdge).collect(Collectors.toSet());
    }
    ...
    public Set<E> incomingEdgesOf(V v) {
        ...
        return edges.stream().filter(this::containsEdge).collect(Collectors.toSet());
    }
    ...
    public Set<V> vertexSet() {
        return graph.vertexSet().stream().filter(vertices).collect(Collectors.toSet());
    }
