...
public record Nary(List<Exp> operands, String name) implements Exp {
    ...
    public Operator operator() {
        List<Type> t = operands.stream().map(Exp::type).toList();
    ...
    public Object value() {
        ...
        List<Object> values = operands.stream().map(Exp::value).toList();
    ...
    public Boolean isConst() {
        return this.operands().stream().allMatch(Exp::isConst);
    }
    ...
    public Exp simplify() {
        ...
        else r = Nary.of(operands.stream().map(Exp::simplify).toList(), this.name());
        return r;
    }
}
