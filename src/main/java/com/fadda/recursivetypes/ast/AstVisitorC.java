...
import com.fadda.recursivetypes.parsers.ProgramBaseVisitor;
import com.fadda.recursivetypes.parsers.ProgramParser;
...
public class AstVisitorC extends ProgramBaseVisitor<Object> {
    ...
    public static final Map<String, Object> symbolTable = new HashMap<>();
     ...
     */
    @Override
    public Declaration visitVarDeclaration(ProgramParser.VarDeclarationContext ctx) {
        String id = ctx.id.getText();
        Type type = Type.valueOf(ctx.type.getText());
        Object value = null;
    ...
    public List<ParamDeclaration> visitFormal_parameters(ProgramParser.Formal_parametersContext ctx) {
        int n = ctx.formal_parameter().size();
     ...
     */
    @Override
    public ParamDeclaration visitFormal_parameter(ProgramParser.Formal_parameterContext ctx) {
        String id = ctx.id.getText();
        Type type = Type.valueOf(ctx.type.getText());
     ...
     */
    @Override
    public Sentence visitIfSentence(ProgramParser.IfSentenceContext ctx) {
     ...
     */
    @Override
    public Exp visitIntExpr(ProgramParser.IntExprContext ctx) {
     ...
     */
    @Override
    public Exp visitBinaryExpr(ProgramParser.BinaryExprContext ctx) {
        String op = ctx.op.getText();
    ...
    @SuppressWarnings("unchecked")
    @Override
    public Exp visitCallExpr(ProgramParser.CallExprContext ctx) {
        String id = ctx.ID().getText();
        List<Exp> parameters = new ArrayList<>();
        if (ctx.real_parameters() != null) parameters = (List<Exp>) visit(ctx.real_parameters());
        Preconditions.checkState(AstVisitorC.symbolTable.containsKey(id),
                String.format("La funci�n %s no ha sido declarada", id));
     ...
     */
    @Override
    public Exp visitDoubleExp(ProgramParser.DoubleExpContext ctx) {
        return Const.of(Double.parseDouble(ctx.getText()), Type.Double);
    }

    @Override
    public Exp visitStrExpr(ProgramParser.StrExprContext ctx) {
    ...
    @Override
    public Exp visitBoolExpr(ProgramParser.BoolExprContext ctx) {
        return Const.of(ctx.getText().equals("true"), Type.Boolean);
    }
    ...
    public Exp visitIdExpr(ProgramParser.IdExprContext ctx) {
        ...
        Preconditions.checkState(AstVisitorC.symbolTable.containsKey(idText),
                String.format("La variable %s no ha sido declarada", idText));
        return (Var) AstVisitorC.symbolTable.get(idText);
    }
     ...
     */
    @Override
    public List<Exp> visitReal_parameters(ProgramParser.Real_parametersContext ctx) {
        int n = ctx.exp().size();
        return IntStream.range(0, n).boxed()
